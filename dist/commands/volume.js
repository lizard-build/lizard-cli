...
export function registerVolume(program) {
    ...
    vol
        ...
        const volumes = await api
            .get(`/api/apps/${service.id}/volumes`)
            .catch(() => []);
        if (isJSONMode()) {
            printJSON(volumes);
            return;
        ...
        table(["Name", "Mount", "Size (MB)", "State"], volumes.map((v) => [v.name, v.mountPath, String(v.sizeMb), v.state || "—"]));
    });
    vol
        .command("add")
        ...
        const created = await api
            .post(`/api/apps/${service.id}/volumes`, {
            name: opts.name,
            ...
            .catch((err) => {
            if (err?.status === 404) {
                throw new Error("Volume endpoint not yet implemented. The API needs " +
        ...
        if (isJSONMode())
            ...
            success(`Volume ${chalk.bold(created.name)} attached at ${created.mountPath}`);
    });
    vol
        .command("delete")
        .alias("rm")
        ...
        await api
            ...
            .catch((err) => {
            if (err?.status === 404) {
                throw new Error("Volume delete endpoint not yet implemented. The API needs " +
                    "`DELETE /api/apps/{id}/volumes/{name}`.");
            }
            throw err;
        });
        if (isJSONMode())
            printJSON({ name, status: "deleted" });
        else
            success(`Volume ${chalk.bold(name)} deleted`);
    });
}
