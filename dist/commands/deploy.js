...
export function registerDeploy(program) {
    program
        ...
        .option("-v, --variable <kv...>", "KEY=value pairs (use Service.KEY=value for service-scoped values)")
        .action(async (opts) => {
        if (!opts.template) {
            warn(`'lizard deploy' now provisions templates. To upload code use ${chalk.bold("lizard up")}.`);
            info(chalk.dim("\nUsage:"));
        ...
        info(chalk.dim(`Template marketplace is coming soon. To create one of the built-in addons use \`lizard add -d ${opts.template}\`.`));
    });
}
