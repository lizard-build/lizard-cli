...
import { isJSONMode, printJSON, table, success, info, warn, timeAgo, isTTY, } from "../lib/format.js";
...
export function registerKeys(program) {
    ...
    keys
        ...
        const created = await api.post("/api/account/api-keys", { name, scopes });
        if (isJSONMode()) {
            printJSON(created);
            return;
        }
    ...
    keys
        ...
        .action(async () => {
        const list = await api.get("/api/account/api-keys");
        if (isJSONMode()) {
            printJSON(list);
            return;
        }
        ...
        table(["Name", "Key", "Scope", "Created", "Last used"], list.map((k) => [
            k.name,
            k.keyPreview,
            k.scopes.length
    ...
    keys
        ...
        const matches = list.filter((k) => k.id.toLowerCase() === lower || k.name.toLowerCase() === lower);
        if (matches.length === 0) {
            throw new Error(`API key "${nameOrId}" not found.`);
        }
        if (matches.length > 1) {
            throw new Error(`Multiple keys named "${nameOrId}". Delete by ID: ${matches.map((m) => m.id).join(", ")}.`);
        }
        ...
        if (isJSONMode())
            printJSON({ id: target.id, status: "deleted" });
        else
            success(`API key ${chalk.bold(target.name)} deleted`);
    });
}
