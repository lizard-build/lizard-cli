...
export function registerEnv(program) {
    ...
    env
        ...
        .option("-s, --service-config <args...>", "Repeatable triple: <SERVICE> <DOT_PATH> <VALUE>")
        .option("-m, --message <text>", "Message to attach to the change")
        .action(async (opts) => {
        const projectId = resolveProjectId(program.opts().project);
        const triples = opts.serviceConfig ?? [];
        if (triples.length === 0) {
            throw new Error("Pass --service-config <SERVICE> <DOT_PATH> <VALUE> (repeatable).");
        }
        ...
        const result = await api.post(`/api/projects/${projectId}/config:apply`, { services });
        if (isJSONMode()) {
            printJSON({
                ok: result?.ok ?? true,
...
function applyDotPath(flat, dotPath, value) {
    ...
    if (!field) {
        throw new Error(`Unknown config path "${dotPath}". Try one of: ${Object.keys(DOT_PATH_FIELD).join(", ")}.`);
    }
    flat[field] = value;
}
function coerceValue(dotPath, raw) {
