...
async function printBalance() {
    const view = await api.get("/api/billing/balance");
    if (isJSONMode()) {
        printJSON(view);
        return view;
    }
    ...
    console.log(chalk.dim("Auto top-up: ") +
...
export function registerCredits(program) {
    ...
    cmd
        ...
        .action(async (opts) => {
        const page = await api.get(withQuery("/api/billing/transactions", {
            limit: opts.limit,
            cursor: opts.cursor,
        ...
        table(["When", "Kind", "Amount", "Balance after", "Description"], page.items.map((t) => [
            timeAgo(t.createdAt),
            t.kind,
    ...
    pm.command("list")
        ...
        .action(async () => {
        const { items } = await api.get("/api/billing/payment-methods");
        if (isJSONMode()) {
            printJSON(items);
            return;
        }
        ...
        table(["ID", "Card", "Expires", "Default"], items.map((m) => [m.id, `${m.brand} •••• ${m.last4}`, `${m.expMonth}/${m.expYear}`, m.isDefault ? "yes" : ""]));
    });
    pm.command("add")
        ...
        .action(async (opts) => {
        const { url } = await api.post("/api/billing/payment-methods/setup", {
    ...
    pm.command("remove <id>")
        ...
        const out = await api.delete(`/api/billing/payment-methods/${encodeURIComponent(id)}`);
        if (isJSONMode()) {
            printJSON(out);
        }
        else {
        ...
        if (opts.method === "x402") {
            ...
            if (opts.quote) {
                if (isJSONMode())
                    ...
                    console.log(`Credits ${fmtCents(quote.creditCents)}, fee ${fmtCents(quote.feeCents)}, total ${fmtCents(quote.totalCents)} USDC on Base`);
                return;
            }
            if (!opts.maxTotal)
                throw new Error("Use --max-total to set a payment limit including fees.");
            const limit = dollarsToCents(opts.maxTotal);
            if (quote.totalCents > limit)
                throw new Error("Payment exceeds --max-total.");
            if (!opts.yes) {
                if (!isTTY())
                    throw new Error("Use --yes and --max-total to authorize an x402 payment.");
            ...
            if (isJSONMode())
                ...
                console.log(`Payment ${result.status}. Attempt: ${result.attemptId}. Request: ${result.requestId}`);
            return;
        }
        if (opts.quote || opts.maxTotal || opts.requestId)
            throw new Error("--quote, --max-total, and --request-id require --method x402.");
    ...
    auto
        ...
        .action(async () => {
        try {
            const out = await api.post("/api/billing/auto-topup/run", {});
            if (isJSONMode()) {
                printJSON(out);
                return;
            }
            ...
            if (e instanceof APIError && e.status === 429) {
                throw new Error(`Auto top-up rate-limited: ${e.body?.reason ?? e.message}`);
            }
            throw e;
        }
    });
    ...
    cmd
        ...
        .action(async (code) => {
        try {
            const out = await api.post("/api/billing/promo/redeem", { code });
            if (isJSONMode()) {
                printJSON(out);
                return;
            }
