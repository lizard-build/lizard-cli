...
export function dollarsToCents(value) {
    if (!/^\d+(\.\d{1,2})?$/.test(value))
        throw new Error('Use a positive dollar amount with at most two decimal places.');
...
async function request(method, route, body, signature) {
    const token = getRequestToken();
    if (!token)
        throw new APIError(401, 'Sign in before buying credits.');
...
async function json(res) {
    ...
    if (!res.ok)
        throw new APIError(res.status, value.error || value.message || 'Payment request failed.', value.error, value);
    return value;
}
export function validateQuote(q, creditCents, maxTotalCents = Number.MAX_SAFE_INTEGER) {
    if (!Number.isSafeInteger(q.creditCents) || !Number.isSafeInteger(q.feeCents) || !Number.isSafeInteger(q.totalCents) ||
        ...
        q.network !== NETWORK || q.asset.toLowerCase() !== ASSET.toLowerCase() || !/^0x[0-9a-fA-F]{40}$/.test(q.payTo))
        throw new Error('Payment quote is invalid or exceeds --max-total.');
}
export async function getX402Quote(creditCents) {
    const q = await json(await request('POST', `${ENDPOINT}/quote`, { creditCents }));
    validateQuote(q, creditCents);
    return q;
}
export function validateChallenge(value, q, origin) {
    ...
    if (value.x402Version !== 2 || value.accepts?.length !== 1 || value.resource?.url !== origin + ENDPOINT || !r ||
        ...
        r.payTo.toLowerCase() !== q.payTo.toLowerCase() || r.amount !== String(BigInt(q.totalCents) * 10000n) ||
        ...
        Object.keys(r.extra || {}).some(key => !['name', 'version', 'assetTransferMethod'].includes(key)) || r.extra?.name !== 'USD Coin' || r.extra?.version !== '2')
        throw new Error('Untrusted x402 payment challenge.');
}
function journalPath(creditCents, requestId) {
    ...
    if (requestId && fs.existsSync(pending)) {
        const previous = JSON.parse(fs.readFileSync(pending, 'utf8'));
...
export async function payX402(creditCents, maxTotalCents, expected, requestId) {
    validateQuote(expected, creditCents, maxTotalCents);
    const origin = paymentOrigin();
    const file = journalPath(creditCents, requestId);
    let state = { requestId: requestId || randomUUID(), creditCents, maxTotalCents, origin };
    try {
        ...
        if (state.origin !== origin || state.creditCents !== creditCents || state.maxTotalCents > maxTotalCents)
            throw new Error('Saved payment differs. Resume with the original amount and limit.');
    ...
    try {
        ...
        if (response.status === 402 && !state.signature) {
            ...
            if (!header || header.length > 16_384)
                throw new Error('Server returned 402 without a valid x402 challenge.');
            ...
            if (body.payTo.toLowerCase() !== expected.payTo.toLowerCase() || body.totalCents !== expected.totalCents)
                throw new Error('Quote changed. Request a fresh quote before paying.');
            validateChallenge(challenge, body, origin);
            const key = process.env.LIZARD_X402_PRIVATE_KEY;
            if (!key || !/^0x[0-9a-fA-F]{64}$/.test(key))
                throw new PaymentInputError('Set LIZARD_X402_PRIVATE_KEY to a funded Base wallet key through your secret manager.');
        ...
        if (e instanceof APIError && e.code === 'QUOTE_EXPIRED')
            throw new Error('Quote expired before payment was accepted. Use a new --request-id UUID.');
...
export async function getX402Status(attemptId) {
    if (!/^[0-9a-f-]{36}$/i.test(attemptId))
        throw new Error('Payment attempt ID must be a UUID.');
    return json(await request('GET', `${ENDPOINT}/${attemptId}`));
}
