...
export function assertValidVolumeName(name) {
    if (VOLUME_NAME_RE.test(name))
        return;
    throw new Error(`Invalid volume name "${name}". Use lowercase letters, digits and dashes, ` +
 ...
 */
export async function resolveVolume(projectId, scope, nameOrId) {
    try {
        return await api.get(withScope(`/api/projects/${projectId}/volumes/${encodeURIComponent(nameOrId)}`, scope));
    }
    catch (e) {
    ...
    const matches = volumes.filter((v) => v.id.toLowerCase() === lower || v.name.toLowerCase() === lower);
    if (matches.length > 1) {
        throw new Error(`Volume "${nameOrId}" is ambiguous — ${matches.length} volumes in this project share that name ` +
