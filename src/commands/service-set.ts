...
export function registerServiceSet(svc: Command) {
  svc
    .command("set")
    .description(
      "Apply build/start/watch/variable changes to one or more services",
    )
    .option("-f, --file <path>", "JSON config file to apply")
    .option(
      "-s, --service-config <args...>",
      "Repeatable: <SERVICE> <DOT_PATH> <VALUE>",
    )
...
async function buildPatch(opts: any, projectId: string): Promise<any> {
...
async function flagsToPatch(flat: string[], projectId: string): Promise<any> {
  if (flat.length % 3 !== 0) {
    throw new Error(
